        ...
        public IActionResult Login()
        {
            if (User.Identity.IsAuthenticated)
            {
                if (User.IsInRole("Öğrenci"))
                {
                    ...
                    HttpContext.Session.Set("DepartmentId", Encoding.ASCII.GetBytes(studentList[0].DepartmentId.ToString()));
        ...
        private async Task Authenticate(LoginModel model, string roleName)
        {
            ...
            else if (username.EndsWith("@ogrenci.ibu.edu.tr"))
            {
                ...
                if (student != null)
                {
                    ...
                    HttpContext.Session.Set("DepartmentId", Encoding.ASCII.GetBytes(student.DepartmentId.ToString()));
                }
            }
            else if (username.EndsWith("@ibu.edu.tr"))
            {
